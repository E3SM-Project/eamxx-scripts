...
def call_job_lister(jobid):
    "Call bjobs. Could be extended to support, e.g., squeue."
    if TESTING:
        sp = Struct()
        ...
        sp = sp_run(['bjobs', str(jobid)])
    if 'is not found' in str(sp.stderr): return JobState.none
    so = str(sp.stdout)
    if ' PEND ' in so: return JobState.pend
    if ' RUN ' in so: return JobState.run
    if ' DONE ' in so: return JobState.done
    if ' CG ' in so: return JobState.completing
    if ' EXIT ' in so: return JobState.exited
    return JobState.unknown

def call_job_killer(jobid):
    "Call bkill. Could be extended to support, e.g., scancel."
    # Run a sequence of kill commands to be maximally convincing.
    for cmd in (['bkill', str(jobid)], ['bkill', '-s 9', str(jobid)],
                ['bkill', '-s 12', str(jobid)]):
        sp = sp_run(cmd)
