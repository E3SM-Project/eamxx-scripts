...
compiler="crayclang-scream" #"craygnuamdgpu" #intel
project="cli115"
walltime="06:00:00"
datestring="20240708"
RUN_REFDATE="2004-05-05"; RUN_REFTOD="00000"
casename=${branch}-${datestring}.${res}.${compset}.pnetcdf   # NOTE: use original casename here but BE CAREFUL!
casebase=${RUN_REFDATE}
...
for file in ${RUNDIR}/rpointer.???; do
    ...
    # Fix restart frequency in atm history files
    #sed -i "s/ndays_x5/ndays_x15/" ${file}
...
for file in ${RUN_REFDIR}/*.rh[0-9].${RUN_REFDATE}-${RUN_REFTOD}.nc; do
    if [ ! -f ${RUNDIR}/`basename ${file}` ]; then
        ln -s ${file} ${RUNDIR}/`basename ${file}`
    fi
done
...
#./case.submit -a="--job-name=decadal-amip -t 02:00:00 --qos=debug --mail-type=ALL --mail-user=bhillma@sandia.gov -x frontier08656 --account=cli115 --gpu-srange=800-1600"
./case.submit -a="--job-name=decadal-amip -t ${walltime} --mail-type=ALL --mail-user=bhillma@sandia.gov -x frontier08656 --account=cli115 --gpu-srange=800-1600"
echo "${caseroot}"
