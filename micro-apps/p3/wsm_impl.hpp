...
class WorkspaceManager<T, DeviceT>::Impl
{
 public:
  ...
  Impl(int size, int max_used, TeamPolicy policy) :
    m_tu(policy),
 ...
 public: // for Cuda
  ...
  static void init(const Impl& wm, const view_2d<T>& data,
 ...
 private: // client should be using Workspace
  ...
  friend class WorkspaceManager::WorkspaceImpl;
...
template <typename T, typename DeviceT>
class WorkspaceManager<T, DeviceT>::WorkspaceImpl
{
 public:
  using Device = DeviceT;
  ...
  KOKKOS_INLINE_FUNCTION
  WorkspaceImpl(const WorkspaceManager::Impl& parent, int ws_idx, const MemberType& team) :
  ...
  Unmanaged<view_1d<S> > take(const char* name) const
  {
    ...
    const auto space = m_parent.template get_space_in_slot<S>(m_ws_idx, m_next_slot);

    // We need a barrier here so get_space_in_slot returns consistent results
    // w/in the team.
    m_team.team_barrier();
    Kokkos::single(Kokkos::PerTeam(m_team), [&] () {
      m_next_slot = m_parent.template get_next<S>(space);
#ifndef NDEBUG
                                   ...
                                   const view_1d_ptr_array<S, N>& ptrs) const
  {
    ...
    for (int n = 0; n < static_cast<int>(N); ++n) {
      const auto space = m_parent.template get_space_in_slot<S>(m_ws_idx, m_next_slot + n);
      micro_kernel_assert(m_parent.template get_next<S>(space) == m_next_slot + n + 1);
    }
#endif
                 ...
                 const view_1d_ptr_array<S, N>& ptrs) const
  {
    ...
    for (int n = 0; n < static_cast<int>(N); ++n) {
      auto& space = *ptrs[n];
      space = m_parent.template get_space_in_slot<S>(m_ws_idx, next_slot);
      next_slot = m_parent.template get_next<S>(space);
    }
  ...
  {
#ifndef NDEBUG
    change_num_used(N - m_parent.template m_num_used(m_ws_idx));
#endif

    for (int n = 0; n < static_cast<int>(N); ++n) {
      const auto space = m_parent.template get_space_in_slot<S>(m_ws_idx, n);
      *ptrs[n] = space;
    }

    // We only need to reset the metadata for spaces that are being left free
    Kokkos::parallel_for(
      Kokkos::TeamThreadRange(m_team, m_parent.template m_max_used - N), [&] (int i) {
        m_parent.template init_metadata(m_ws_idx, i+N);
      });
    ...
    Kokkos::single(Kokkos::PerTeam(m_team), [&] () {
        ...
        for (int a = 0; a < m_parent.template m_max_used; ++a) {
          if (m_parent.template m_active(m_ws_idx, a)) {
            change_indv_meta<S>(m_parent.template get_space_in_slot<S>(m_ws_idx, a), "", true);
          }
        }
  ...
  void reset() const
  {
    m_team.team_barrier();
#ifndef NDEBUG
    change_num_used(-m_parent.template m_num_used(m_ws_idx));
#endif
    m_next_slot = 0;
    Kokkos::parallel_for(
      Kokkos::TeamThreadRange(m_team, m_parent.template m_max_used), [&] (int i) {
        m_parent.template init_metadata(m_ws_idx, i);
      });
    ...
    Kokkos::single(Kokkos::PerTeam(m_team), [&] () {
        ...
        for (int a = 0; a < m_parent.template m_max_used; ++a) {
          if (m_parent.template m_active(m_ws_idx, a)) {
            change_indv_meta<T>(m_parent.template get_space_in_slot<T>(m_ws_idx, a), "", true);
          }
        }
      });
#endif
  ...
  void print () const
  {
    ...
    Kokkos::single(
      Kokkos::PerTeam(m_team), [&] () {
        std::stringstream ss;
        ss << m_ws_idx << ":";
        auto space = m_parent.template get_space_in_slot<T>(m_ws_idx, m_next_slot);
        for (int cnt = 0, nmax = m_parent.template m_max_used;
             cnt < nmax;
             ++cnt) {
          ss << " (" << m_parent.template get_index<T>(space) << ", "
             << m_parent.template get_next<T>(space) << ")";
          space = m_parent.template get_space_in_slot<T>(m_ws_idx, m_parent.template get_next<T>(space));
        }
        ss << "\n";
        std::cout << ss.str();
      });
...
private:
  ...
  {
    const int slot = m_parent.template get_index<S>(space);
    return &(m_parent.template m_curr_names(m_ws_idx, slot, 0));
  }
  ...
  {
    Kokkos::single(Kokkos::PerTeam(m_team), [&] () {
        int curr_used = m_parent.template m_num_used(m_ws_idx) += change_by;
        micro_kernel_assert(curr_used <= m_parent.template m_max_used);
        micro_kernel_assert(curr_used >= 0);
        if (curr_used > m_parent.template m_high_water(m_ws_idx)) {
          m_parent.template m_high_water(m_ws_idx) = curr_used;
        }
      });
  }
  ...
  void change_indv_meta(const Unmanaged<view_1d<S> >& space, const char* name, bool release=false) const
  {
    Kokkos::single(Kokkos::PerTeam(m_team), [&] () {
        ...
        if (!release) {
          ...
          micro_kernel_assert(!m_parent.template m_active(m_ws_idx, slot));
          char* val = &(m_parent.template m_curr_names(m_ws_idx, slot, 0));
        ...
        m_parent.template m_counts(m_ws_idx, name_idx, count_idx) += 1;
        m_parent.template m_active(m_ws_idx, slot) = !release;
      });
  }

  KOKKOS_INLINE_FUNCTION
  int get_alloc_count(const char* name) const
  { return m_parent.template m_counts(m_ws_idx, get_name_idx(name), 0); }

  KOKKOS_INLINE_FUNCTION
  int get_release_count(const char* name) const
  { return m_parent.template m_counts(m_ws_idx, get_name_idx(name), 1); }

  KOKKOS_INLINE_FUNCTION
  int get_num_used() const
  { return m_parent.template m_num_used(m_ws_idx); }

  template <typename S>
  KOKKOS_INLINE_FUNCTION
  bool is_active(const Unmanaged<view_1d<S> >& space) const
  { return m_parent.template m_active(m_ws_idx, m_parent.template get_index<S>(space));}
  ...
  int get_name_idx(const char* name, bool add=false) const
  {
    int name_idx = -1;
    for (int n = 0; n < m_max_names; ++n) {
      char* old_name = &(m_parent.template m_all_names(m_ws_idx, n, 0));
  ...
  void release_impl(const Unmanaged<view_1d<S> >& space) const
  {
    ...
    Kokkos::single(Kokkos::PerTeam(m_team), [&] () {
        m_next_slot = m_parent.template set_next_and_get_index<S>(space, m_next_slot);
      });
    m_team.team_barrier();
  }

  enum { m_pad_factor   = WorkspaceManager::Impl::m_pad_factor,
         m_max_name_len = WorkspaceManager::Impl::m_max_name_len,
         m_max_names    = WorkspaceManager::Impl::m_max_names
  };

  const WorkspaceManager::Impl& m_parent;
...
{
  impl = std::make_shared<Impl>(size, max_used, policy);
}
...
WorkspaceManager<T, D>::get_workspace(const typename WorkspaceManager<T, D>::MemberType& team) const
{
  return impl->get_workspace(team);
}

template <typename T, typename D>
KOKKOS_INLINE_FUNCTION
WorkspaceManager<T, D>::Workspace::Workspace(const typename WorkspaceManager<T, D>::Impl& parent,
                                             int ws_idx,
                                             const typename WorkspaceManager<T, D>::MemberType& team)
{
  impl = std::make_shared<typename WorkspaceManager<T, D>::WorkspaceImpl>(parent, ws_idx, team);
}
...
WorkspaceManager<T, D>::Workspace::take(const char* name) const
{
  return impl->template take<S>(name);
}
...
KOKKOS_INLINE_FUNCTION
void
WorkspaceManager<T, D>::Workspace::take_many_contiguous_unsafe(const Kokkos::Array<const char*, N>& names,
                                                               const typename WorkspaceManager<T, D>::template view_1d_ptr_array<S, N>& ptrs) const
{
  impl->template take_many_contiguous_unsafe<N, S>(names, ptrs);
}
...
KOKKOS_INLINE_FUNCTION
void
WorkspaceManager<T, D>::Workspace::take_many(const Kokkos::Array<const char*, N>& names,
                                             const typename WorkspaceManager<T, D>::template view_1d_ptr_array<S, N>& ptrs) const
{
  impl->template take_many<N, S>(names, ptrs);
}
...
KOKKOS_INLINE_FUNCTION
void
WorkspaceManager<T, D>::Workspace::take_many_and_reset(const Kokkos::Array<const char*, N>& names,
                                                       const typename WorkspaceManager<T, D>::template view_1d_ptr_array<S, N>& ptrs) const
{
  impl->template take_many_and_reset<N, S>(names, ptrs);
}
...
KOKKOS_FORCEINLINE_FUNCTION
void
WorkspaceManager<T, D>::Workspace::release(const View& space, std::enable_if<(View:: rank == 1)>*) const
{
  impl->release(space);
}
...
KOKKOS_INLINE_FUNCTION
const char*
WorkspaceManager<T, D>::Workspace::get_name(const View& space, std::enable_if<(View:: rank == 1)>*) const
{
  return impl->get_name(space);
}
#endif
...
KOKKOS_INLINE_FUNCTION
void
WorkspaceManager<T, D>::Workspace::reset() const
{
  impl->reset();
}
...
template <typename T, typename D>
void
WorkspaceManager<T, D>::Workspace::print() const
{
  impl->print();
}
